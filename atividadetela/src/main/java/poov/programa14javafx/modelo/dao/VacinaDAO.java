...
public class VacinaDAO extends GenericJDBCDAO<Vacina, Long> {
    ...
    public List<Vacina> buscar(String codigo, String nome, String descricao) {
        try {
            ...
            if (!nome.isEmpty()) {
                sql += "upper(nome) like upper(?) AND ";
            }
            if (!descricao.isEmpty()) {
                sql += "upper(descricao) like upper(?) AND ";
            }
            sql += "SITUACAO = 'ATIVO'";
            ...
            if (!descricao.isEmpty()) {
                statement.setString(indice, "%" + descricao + "%");
                indice++;
            }
            ResultSet resultSet = statement.executeQuery();
            return toEntityList(resultSet);
